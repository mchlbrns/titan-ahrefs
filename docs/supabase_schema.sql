...
-- Turn on Row Level Security (RLS) and allow anon read/write
ALTER TABLE public.managed_domains ENABLE ROW LEVEL SECURITY;

CREATE POLICY "Allow public select on managed_domains" 
  ON public.managed_domains FOR SELECT USING (true);

CREATE POLICY "Allow public insert/update/delete on managed_domains" 
  ON public.managed_domains FOR ALL USING (true);
...
CREATE POLICY "Allow public select on ahrefs_snapshots" 
  ON public.ahrefs_snapshots FOR SELECT USING (true);

CREATE POLICY "Allow public insert/update on ahrefs_snapshots" 
  ON public.ahrefs_snapshots FOR ALL USING (true);
